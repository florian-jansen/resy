...
.resy_assign_ecoregions <- function(data) {
  utils::data("ecoregions2017_epsg25832", package = "RESY", envir = environment())
  ecoregions_sf <- get("ecoregions2017_epsg25832", inherits = FALSE)

  data |>
    sf::st_join(ecoregions_sf, left = TRUE, largest = FALSE) |>
    dplyr::select(
      -dplyr::any_of(c("OBJECTID", "BIOME_NUM", "BIOME_NAME", "REALM", "COLOR", "LICENSE")),
...
.resy_assign_country <- function(data) {
  ...
  utils::data("europe_resolution_60_epsg25832", package = "RESY", envir = environment())
  ...
  data |>
    ...
    dplyr::mutate(Country = dplyr::recode(Country, !!!.country_map, .default = Country))
}
...
.resy_assign_coast_dunes <- function(data_sf, buffer_dist = 5000) {
  ...
  if (!is.numeric(buffer_dist) || length(buffer_dist) != 1 || buffer_dist <= 0)
    stop("`buffer_dist` must be a single positive number (metres).", call. = FALSE)
  ...
  dune_flag <- ifelse(rowSums(sf::st_intersects(data_sf, dunes, sparse = FALSE)) > 0,
                      "Y_DUNES", "N_DUNES")
